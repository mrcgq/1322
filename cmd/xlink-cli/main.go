...
func main() {
	...
	serverIP    := flag.String("ip",       "",                  "Specific server IP (passed to Worker as ?pyip=)")
	secretKey   := flag.String("key",      "",                  "Secret key (pure, no | contamination)")
	socks5Addr  := flag.String("s5",       "",                  "SOCKS5 proxy address")
	fallbackAddr := flag.String("fallback", "",                 "Fallback relay IP (passed to Worker as Nano fb field + ?pyip= fallback)")
	...
	configJSON := core.GenerateConfigJSON(
		...
		*socks5Addr, *fallbackAddr, *listenAddr,
		*strategy, *rules,
	)

	log.Println("[CLI] Starting X-Link Odyssey Kernel (v14.1)...")
